...
class Pdf extends MY_Controller{
  ...
  function listaPrecios(){
    ...
    $articulos=$this->Articlos_model->getAll();
    $this->fpdf->Open();
    ...
    foreach($articulos as $articulo){
      if($hoja!=$hojaAux){
        ...
        $this->fpdf->Cell(20,6,"Precio",1,1,"C",true);
        $hojaAux=$hoja;
      };
      $this->fpdf->Cell(20,6,sprintf("%04f",$articulo->id),1,0,'R');
      $this->fpdf->Cell(80,6,substr($articulo->nombre,0,40),1,0,'L');
      $this->fpdf->Cell(20,6,sprintf("$%4.2f",$articulo->precio),1,1,'R');
